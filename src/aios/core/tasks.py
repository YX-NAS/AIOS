...
def plan_goal(root: Path, goal: str, priority: str = "high", create: bool = True) -> list[dict]:
    ...
    template = build_goal_template(goal, base_type)
    tasks = load_tasks(root)
    planned: list[dict] = []
    ...
    for item in template:
        ...
        record = build_task_record(
            root,
            tasks + planned,
            title_template.format(goal=goal_summary),
...
def build_goal_template(goal: str, base_type: str) -> list[tuple]:
    lowered = goal.lower()
    if base_type == "bug_fix" and any(keyword in lowered for keyword in ["时间", "状态", "上下文", "承接", "同步"]):
        return [
            (
                ...
                "bug_fix",
                "high",
                ["明确错误表现", "列出受影响的状态或文案", "确认需要保持不变的既有行为"],
            ),
            (
                "排查时间锚点与承接逻辑：{goal}",
                "bug_fix",
                "high",
                ["定位真实时间来源", "定位旧上下文继续承接的入口", "确认根因链路"],
            ),
            (
                "修复时间状态同步：{goal}",
                "bug_fix",
                "high",
                ["状态根据当前真实时间更新", "动作、内容、语言与时间一致", "避免沿用数小时前的状态"],
            ),
            (
                "补充跨时段回归验证：{goal}",
                "testing",
                "high",
                ["覆盖同小时、跨小时、隔夜场景", "验证旧会话不会污染当前时间状态", "给出验证步骤或测试结果"],
            ),
            (
                "更新记录与验收说明：{goal}",
                "documentation",
                "medium",
                ["记录根因和修复点", "记录验证范围", "补充后续观察项"],
            ),
        ]
    if base_type == "complex_coding" and looks_like_system_build_goal(goal):
        return build_system_goal_template(goal)
    return GOAL_TEMPLATES.get(base_type, GOAL_TEMPLATES["complex_coding"])
...
def build_system_goal_template(goal: str) -> list[tuple]:
    ...
    template: list[tuple] = [
        (
            ...
            "architecture",
            "high",
            ["明确系统目标", "列出核心模块", "说明本阶段不做的范围"],
        ),
        (
            "设计核心数据与接口：{goal}",
            "architecture",
            "high",
            ["明确核心数据结构", "明确模块之间的接口", "说明关键约束和异常处理"],
        ),
    ]
    if modules:
    ...
    return apply_goal_summary(template, system_name)


def apply_goal_summary(template: list[tuple], goal_summary: str) -> list[tuple]:
    adjusted: list[tuple] = []
    for item in template:
        title_template, task_type, default_priority = item[:3]
        acceptance = item[3] if len(item) > 3 else None
        adjusted.append((title_template.replace("{goal}", goal_summary), task_type, default_priority, acceptance))
    return adjusted
