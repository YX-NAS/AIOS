# AIOS 多模型开发中枢 v0.42.0

AIOS 是一个本地文件系统型本地开发中枢，当前同时提供 CLI 和 Web UI，用来为软件项目生成 `.aios/` 知识目录、扫描项目结构、管理开发任务、推荐模型路由，并生成可复制给不同 AI 模型的 Context Pack。

## v0.42.0 MVP 业务主链更新

本轮将既有的任务、路由、执行记录和多项目总控台串成默认推进链：

`目标 -> 任务树 -> 当前任务 -> 执行 -> 回写 -> 下一任务 -> Launcher 摘要`

- **项目 Goal**：每个项目同一时间只保留一个活动目标，持久化到 `.aios/goals.json`
- **任务树绑定**：通过 Goal 创建的任务带稳定 `goal_id`、父任务、依赖和顺序信息
- **唯一当前任务**：项目页自动优先展示执行中、待验收或可执行的下一任务
- **自动推进**：`run finish`、`complete` 和 Web 完成回写后会重新计算目标状态与下一任务
- **推进视图**：单项目页新增目标完成度和“现在该做什么”；Launcher 项目卡同步显示当前目标、当前任务、进度和最近推进时间
- **兼容旧项目**：已有任务和 `.aios/` 无需迁移；只有新 Goal 的任务会进入推进主链
...
- **pipeline 步骤编排**：`run_auto_pipeline_step` 会先为 ready 任务生成执行记录、Pack 和交接单，再串联 switch → build → execute
...
aios goal create "完成聊天接口时间上下文修复"
aios goal list
aios goal advance
...
- 显式确认后打开 CC Switch，导入当前任务并打开终端恢复命令
- 挂接任务执行会话并复制恢复命令
- 查看历史会话候选并一键挂接
...
- 查看每个项目的当前 Goal、当前任务、推进进度和下一步动作
