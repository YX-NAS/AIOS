...
当前版本已交付到 `P3-35` 的全部能力，核心功能包括：

- 140 个自动化测试全部通过
- 完整的多项目启动器 + 单项目 Web UI
- 半自动执行链路：目标拆解 → 模型路由 → Pack 生成 → 手动执行 → 完成回写
- 全自动恢复链：失败分类 → 差异化恢复 → 多轮恢复 + 冷却护栏
- CLI 模式 bypass ccswitch bridge（自动化执行核心突破）
- 预算策略 + 成本统计 + cheapest_first 派发
- Session 挂接 / 终端续接 / ccswitch bridge / Provider 探测
- Git auto-commit / auto-push / auto PR
...
- 仍需人工切换 `ccswitch`（P4-0 将解决）
- 尚未自动调用 Codex / Claude Code CLI（P4-1 将解决）
- 尚未实现端到端全自动 pipeline（P4-2 将解决）
- 缺少任务级超时和人工接管队列（P4-3 将解决）
...
## P4 全自动调度闭环（2026-07-03 启动）

详见 [docs/P4_DESIGN.md](P4_DESIGN.md)。

### P4 子阶段

| 子阶段 | 目标 | 预期版本 |
|--------|------|----------|
| P4-0 | ccswitch 自动切换 | v0.39 |
| P4-1 | Codex / Claude Code 自动调用 | v0.40 |
| P4-2 | 端到端闭环调度 | v0.41 |
| P4-3 | 质量保障与人工接管 | v0.42 |

### 关键决策记录

| 决策时间 | 决策内容 | 原因 |
|----------|----------|------|
| 2026-07-03 | CLI 模式 bypass ccswitch bridge 是自动化的核心突破口 | CLI 通过 --model flag 指定模型，不再需要 UI 级切换 |
| 2026-07-03 | cswitch Deep Link API 是自动切换的首选路径 | 已有稳定公开 API，比 Computer Use 更可靠 |
| 2026-07-03 | P4 先做单任务全自动闭环，再做跨任务流水线 | 降低风险，逐步验证 |
