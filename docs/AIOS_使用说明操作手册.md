...
aios run auto --executor codex-cli --auto-recover-failures
...
如果你希望系统统计模型成本，需要先在 launcher 的全局模型库里补价格：
...
- Context Pack 文本估算出的 prompt token
- 执行输出估算出的 output token
- 模型库里维护的单价
...
当你开始使用 `aios run auto` 或 Web 里的“自动推进下一步”时，建议先配置预算策略。

入口：

- 单项目 Web UI 的“项目状态”区域
- 模块名：`自动化预算策略`
...
- `default`：沿用当前调度顺序
- `cheapest_first`：在多个 `ready` 任务里，优先自动派发预计成本更低的任务

预算阻塞后，任务不会进入自动执行，而会在任务检查器的“调度状态”里显示：
...
- 如果你希望在验证失败时，AIOS 自动切到下一候选模型再试一次，可以追加：

```bash
aios --root /path/to/project run TASK-20260701-001 --executor codex-cli --auto-finish --summary "完成登录修复" --verify-command "pytest -q" --retry-on-verify-fail
```

- 新版也支持更通用的自动恢复开关：

```bash
aios --root /path/to/project run TASK-20260701-001 --executor codex-cli --auto-finish --summary "完成登录修复" --verify-command "pytest -q" --auto-recover-failures
```

- 当前恢复策略：
  - 验证失败：切到下一候选模型再试一次
  - 执行超时 / 网络不可达 / 普通非零退出：同模型自动重跑一次
  - provider 鉴权失败：不会自动恢复，会要求先修配置
- 你还可以在项目控制台的预算策略区域设置“自动恢复次数上限”。
- 默认值是 `2`，表示最多连续自动恢复两轮。
- 达到上限后，AIOS 会停止继续恢复，等待人工接管。
...
### 第 7.9 步：检查模型 provider / 鉴权是否就绪

在执行器可用之后，建议再检查一次模型侧是否具备调用条件：
...
当前版本中，launcher 还会直接显示：

- 每个项目卡片的 provider 就绪数
- 每个模型的运行时状态
- 缺少哪些鉴权环境变量

这能帮助你在开始执行前，先判断“推荐模型是否真的在本机可用”。
