...
**当前版本范围：** AIOS 已把“项目目标 -> 任务树 -> 当前任务 -> 执行 -> 完成回写 -> 下一任务”串成默认工作流。人工切换 ccswitch 的半自动流程仍然可用；执行器自动化能力继续受本机配置和项目策略约束。
...
### 步骤 3：创建项目目标并生成任务树（推荐）

在单项目 Web UI 的「任务台」输入一句完整目标，点击「创建并拆分目标」。AIOS 会：

1. 创建一个项目 Goal；
2. 自动生成带依赖关系的任务树；
3. 计算唯一的当前任务；
4. 在「项目推进」面板展示完成度、推荐模型和下一步动作。

V1 每个项目同一时间只允许一个活动 Goal。完成或处理完阻塞后，才能新建下一目标。

CLI 等价命令：

```bash
aios --root /path/to/project goal create "添加用户画像模块，包含数据采集、存储、分析、前端展示"
aios --root /path/to/project goal list
aios --root /path/to/project goal advance
```
...
旧式 `aios task plan` 仍可用于只生成任务列表或草案；这类任务不会自动绑定到项目 Goal。
...
- `changelog.md` 追加完成记录
- `memory.md` 追加经验备忘
- 当前 Goal 的完成进度和下一条可执行任务
- Launcher 首页对应项目的推进摘要
...
1. 在 AIOS 中点击「复制 ccswitch Deep Link」或运行 CLI 导出；此操作只生成或复制链接，不会打开 ccswitch。
2. 需要导入时，再由你打开该链接，或在 AIOS 点击「打开 CC Switch 并导入当前任务」。后者会先显示确认框。
3. 确认后，AIOS 才会依次打开 provider 和 prompt 的导入链接，并打开终端恢复命令。
4. 在 ccswitch 中确认模型和会话正确后，在 Codex 或 Claude Code 执行任务。
5. 完成后回到 AIOS 填写测试结果和完成总结。

**注意：** `resource=prompt` 的内容会以 Base64 后再进行 URL 转义，避免 `+`、`=` 等字符在 Deep Link 查询参数中损坏。桥接动作仍需要人工确认结果，AIOS 不会静默改变你当前桌面会话。

使用 `aios run pipeline --auto-switch` 时，AIOS 也会先为尚未准备执行的当前任务生成执行记录、Context Pack 和交接单，再打开 Deep Link；不需要先额外运行一次 `aios run --manual`。
